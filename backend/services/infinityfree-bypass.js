...
function expandKey(i, t) {
  for (var r = 16 * (numberOfRounds(t) + 1), o = 0, n = 1, s = [], e = [], a = 0; a < r; a++) e[a] = 0;
  for (var h = 0; h < t; h++) e[h] = i[h];
  for (o += t; o < r;) {
    for (var u = 0; u < 4; u++) s[u] = e[o - 4 + u];
    if (o % t === 0 && (s = core(s, n++)), t === 32 && o % t === 16)
      for (var f = 0; f < 4; f++) s[f] = sbox[s[f]];
...
function aes_decrypt(i, t, r) {
  for (var o = [], n = [], s = numberOfRounds(r), e = 0; e < 4; e++) for (var a = 0; a < 4; a++) n[e + 4 * a] = i[4 * e + a];
  r = expandKey(t, r); n = invMain(n, r, s);
  for (e = 0; e < 4; e++) for (a = 0; a < 4; a++) o[4 * e + a] = n[e + 4 * a];
  return o;
}
...
function slowaesDecrypt(t, o, n) {
  var s = o.length;
  var e, a = [], h = [], u = [], f = [], l = !0;
  if (null !== t) {
    for (var c = 0; c < Math.ceil(t.length / 16); c++) {
      var d = 16 * c, p = 16 * c + 16;
      if (16 * c + 16 > t.length && (p = t.length), e = getBlock(t, d, p), 2 == 2) {
        for (h = aes_decrypt(e, o, s), i = 0; i < 16; i++) u[i] = (l ? n : a)[i] ^ h[i];
        l = !1;
...
function computeCookie(aHex, bHex, cHex) {
  const a = toNumbers(aHex);
  const b = toNumbers(bHex);
  const c = toNumbers(cHex);
  return toHex(slowaesDecrypt(c, a, b));
}

function isChallengeResponse(body) {
  return body.includes('slowAES') && body.includes('toNumbers');
}

function extractChallengeParams(body) {
  const parts = body.match(/toNumbers\("([^"]+)"\)/g);
...
function isAntibotBlocked(statusCode, body) {
  return statusCode === 400 && typeof body === 'string' && body.includes('openresty');
}

function getCacheKey(parsedUrl) {
  return parsedUrl.protocol + '//' + parsedUrl.hostname;
}

async function getCookie(parsedUrl) {
  const cacheKey = getCacheKey(parsedUrl);
  ...
  const challengeUrl = `${parsedUrl.protocol}//${parsedUrl.hostname}/api/setup.php`;
  const result = await makeRequest(new URL(challengeUrl), 'GET', {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0'
  });

  let cookie;
  if (isChallengeResponse(result.body)) {
    const params = extractChallengeParams(result.body);
    if (params) {
      cookie = computeCookie(params.a, params.b, params.c);
    }
  }
...
async function requestWithAntiBotBypass(urlStr, options = {}) {
  const parsedUrl = new URL(urlStr);
  const method = options.method || 'GET';
  const reqBody = options.body || null;
  const reqHeaders = { ...(options.headers || {}), 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0' };

  const cookie = await getCookie(parsedUrl);
  if (cookie) {
    reqHeaders['Cookie'] = `__test=${cookie}`;
  }

  let result = await makeRequest(parsedUrl, method, reqHeaders, reqBody);

  if (isAntibotBlocked(result.statusCode, result.body) || isChallengeResponse(result.body)) {
    const freshCookie = await getCookie(parsedUrl);
    if (freshCookie) {
      reqHeaders['Cookie'] = `__test=${freshCookie}`;
      result = await makeRequest(parsedUrl, method, reqHeaders, reqBody);
    }
  }
...
module.exports = { requestWithAntiBotBypass, computeCookie };
